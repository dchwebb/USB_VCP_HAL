...
volatile uint8_t uartCmdSize = 0;
volatile char uartCmd[50];
volatile uint8_t uartCmdRdy = 0;
...
//			for (int i = 1; i < 3; i++) {
//				//CDC_Transmit_FS((uint8_t*)uartCmd, uartCmdSize);
//				uint8_t result = CDC_Transmit_FS((uint8_t*)transmit, strlen(transmit));
//				while (result == USBD_BUSY) {
//					result = CDC_Transmit_FS((uint8_t*)transmit, strlen(transmit));
//				}
//			}
